...
public class Baekjoon1753_최단경로 {
	static class Edge implements Comparable<Edge> {
		...
		private Edge(int weight, int end) {
			this.weight = weight;
			this.end = end;
		}
	...
	private static String solution(int vertex, int edge, int start, int[][] numbers) {
		...
		while (!pq.isEmpty()) {
			// 2. 우선순위 큐에서 거리가 가장 작은 원소를 pop하여 선택하고, 원소의 거리가 최단 거리 배열의 값과 같은지 확인
			Edge selectEdge = pq.poll();
			if (distance[selectEdge.end] != selectEdge.weight) {
				continue;
			}
			...
			for (Edge nearEdge : graph[selectEdge.end]) {
				...
				if (min != distance[nearEdge.end]) {
					distance[nearEdge.end] = min;
					pq.add(new Edge(distance[nearEdge.end], nearEdge.end));
				}
			}
		}
